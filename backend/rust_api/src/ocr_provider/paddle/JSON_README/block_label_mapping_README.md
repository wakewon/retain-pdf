...
这份文档基于 [json_full.json](/workspace/example-project/backend/rust_api/src/ocr_provider/paddle/json_full.json) 的 `layoutParsingResults[*].prunedResult.parsing_res_list[*].block_label` 实际枚举结果整理，目标是给后续 `Paddle -> document.v1` adapter 提供第一版稳定映射。
