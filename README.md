...
Docker 模式默认可选择 `mineru_local`，通过宿主机或内网的 MinerU API bridge 完成本地 OCR。推荐在已有 uv/conda MinerU 环境中启动 `tools/mineru_api_bridge.py`，Docker 后端默认访问 `http://host.docker.internal:18080`。如仍需把 MinerU 放进 Docker，可启用 `mineru-sidecar` profile。
